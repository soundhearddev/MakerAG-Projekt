...
(function () {
  ...
  // Escaped beliebigen Text für sicheres Einfügen via innerHTML.
  // Genutzt für alle Felder die potenziell Freitext aus der DB sind
  // (notes, tags, specs-Werte) — auch wenn sie aktuell nicht direkt
  // von Nutzern editierbar sind, kann sich das ändern (siehe edit-*.php
  // Endpoints, die schon ohne Login Item-Daten verändern können).
  function esc(value) {
  ...
  fetch(`/api/fetch_from_id.php?id=${encodeURIComponent(id)}`)
    ...
    .then((data) => {
      ...
      if (item.serial_number) appendIf(statusDiv, infoLine(
        "Seriennummer", `<span class="item-serial">${esc(item.serial_number)}</span>`,
      ));
      ...
      if (item.notes) appendIf(statusDiv, infoLine(
        "Notizen", `<span class="item-notes">${esc(item.notes)}</span>`,
      ));
      ...
      if (specs && !Array.isArray(specs) && typeof specs === "object" && Object.keys(specs).length) {
        ...
        Object.entries(specs).forEach(([key, value]) => {
          ...
          li.innerHTML = `<strong>${esc(key)}:</strong> ${esc(value)}`;
        ...
        specs.forEach((spec) => {
          const li = document.createElement("li");
          li.innerHTML = `<strong>${esc(spec.key || spec.label)}:</strong> ${esc(spec.value)}`;
          specsList.appendChild(li);
        });
      } else {
